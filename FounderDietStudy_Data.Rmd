...
```{r}
plasmafile <- linkpath("PlaMet", links)
...
```{r}
liverfile <- linkpath("LivRna", links)
```
...
```{r}
LivMetfile <- linkpath("LivMet", links)
```

```{r}
if(file.exists(rdsfile <- "LivMetData.rds")) {
  LivMetData <- readRDS(rdsfile)
} else {
  LivMetData <- read_excel(LivMetfile, skip = 3) %>%
    pivot_longer(-(1:5), names_to = "mousecode", values_to = "value") %>%
    mutate(
      strain = str_remove(str_extract(mousecode, "^.*-"), "-"),
      animal = str_extract(str_remove(mousecode, "^.*-"), "^[0-9]+"),
      sex = str_extract(str_remove(mousecode, "^.*-[0-9]+_"), "^[MF]"),
  ...
  saveRDS(LivMetData, rdsfile)
}
rm(rdsfile)
```

```{r}
if(file.exists(rdsfile <- "LivMetSignal.rds")) {
  LivMetSignal <- readRDS(rdsfile)
} else {
  LivMetSignal <- partition(LivMetData)
  
  saveRDS(LivMetSignal, rdsfile)
}
rm(rdsfile)
```

```{r}
if(file.exists(rdsfile <- "LivMetStats.rds")) {
  LivMetStats <- readRDS(rdsfile)
} else {
  LivMetStats <- strainstats(LivMetData)

  saveRDS(LivMetStats, rdsfile)
}
rm(rdsfile)
```
...
traitStats <- dplyr::bind_rows(
  ...
  LivMet = readRDS("LivMetStats.rds"),
...
traitSignal <- dplyr::bind_rows(
  ...
  LivMet = readRDS("LivMetSignal.rds"),
