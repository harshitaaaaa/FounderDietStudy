...
```{r}
traits <- traitPvalue$trait[seq_len(as.integer(params$traits))]
meas <- meas %>% 
...
ggplot(traitPvalue %>% arrange(p_signal)) +
...
```{r}
physiosum <- (traitPvalue %>% filter(datatype == "physio"))
```
...
```{r}
plasmasum <- readRDS("traitPvalue.rds") %>%
...
```{r}
liverPvalue <- readRDS("traitPvalue.rds") %>%
